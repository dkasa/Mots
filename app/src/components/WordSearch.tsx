...
export function WordSearch({ allWords, darkMode = false, onSync, onToggle }: WordSearchProps) {
  ...
  const getCurrentCombinationIndex = () => {
    const expandedWords = selectedWord ? getExpandedWords(selectedWord.word.french) : [];
  ...
  const playAudio = () => {
    ...
    const expandedWords = getExpandedWords(selectedWord.word.french);
    ...
    const audioPath = `/audio/grade${selectedWord.word.grade}/${currentGender}/${filename}.m4a`;
    ...
    audio.play().catch(error => {
      ...
      setTimeout(() => {
        ...
        const fallbackAudioPath = `/audio/grade${selectedWord.word.grade}/${fallbackGender}/${fallbackFilename}.m4a`;
  ...
  // 检查单词是否包含逗号
  const hasComma = selectedWord && selectedWord.word.french.includes(',');
